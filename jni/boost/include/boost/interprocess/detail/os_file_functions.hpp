...
typedef loff_t     offset_t;   //MGJANG_OFF_T
...
//   if(sizeof(off_t) == sizeof(std::size_t)){
  if(sizeof(loff_t) == sizeof(std::size_t)){  // MGJANG_OFF_T
...
//   return 0 == ::ftruncate(hnd, off_t(size));
	return 0 == ::ftruncate(hnd, loff_t(size));  // MGJANG_OFF_T
}
...
{  return ((loff_t)(-1)) != ::lseek(hnd, off, (int)pos); }  // MGJANG_OFF_T
...
{  
   ...
   //return off != ((off_t)-1);
   return off != ((loff_t)-1); // MGJANG_OFF_T
}
