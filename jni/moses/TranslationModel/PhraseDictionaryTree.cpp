...
public:
  ...
  LOGE("[mgjang] in GetTargetCandidates function \n");
    if(f.empty()) return;
...
// mgjang check
    ...
    if(tCandOffset==InvalidOffT) 
	{
		LOGE("[mgjang] tCandOffset==InvalidOffT \n");
		return;
    }
    ...
    if (HasAlignmentInfo())
      ...
      tgtCands.readBin(ot);
	LOGE("[mgjang] end of GetTargetCandidates function \n");
  }
...
{
  	LOGE("[mgjang] in PhraseDictionaryTree function \n");
	LOGE("[mgjang] sizeof OFF_T = %d\n", sizeof(OFF_T));
	LOGE("[mgjang] sizeof off_t = %d\n", sizeof(off_t));
#if 1
  if(sizeof(OFF_T)!=8) {
  	LOGE("[mgjang] OFF_T != 8\n");
    TRACE_ERR("ERROR: size of type 'OFF_T' has to be 64 bit!\n"
              ...
              " -> abort \n\n");
  	LOGE("[mgjang] before abort(); in PhraseDictionaryTree function \n");
//    abort();
  }
#endif
}
...
GetTargetCandidates(const std::vector<std::string>& src,
                    ...
                    std::vector<std::string>& wa) const
{
	LOGE("[mgjang] in GetTargetCandidates \n");
  IPhrase f(src.size());
  for(size_t i=0; i<src.size(); ++i) {
  	LOGE("[mgjang] in IPhrase loop\n");
    f[i]=imp->sv->index(src[i]);
    if(f[i]==InvalidLabelId) return;
  }
