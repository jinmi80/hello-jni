...
bool PhraseDictionaryTreeAdaptor::Load(const std::vector<FactorType> &input
                                       ...
                                       , float weightWP)
{
	LOGE("[mgjang] in PhraseDictionaryTreeAdaptor::Load function\n");
  if(m_numScoreComponent!=weight.size()) {
	LOGE("[mgjang] 1 PhraseDictionaryTreeAdaptor::Load function\n");  	
    std::stringstream strme;
    strme << "ERROR: mismatch of number of scaling factors: "<<weight.size()
          <<" "<<m_numScoreComponent<<"\n";
    UserMessage::Add(strme.str());
	LOGE("[mgjang] 2 PhraseDictionaryTreeAdaptor::Load function\n");	
    return false;
  }
  ...
  LOGE("[mgjang] 3 PhraseDictionaryTreeAdaptor::Load function\n");

  imp->Create(input,output,filePath,weight,languageModels);
	LOGE("[mgjang] 4 PhraseDictionaryTreeAdaptor::Load function\n");  
  return true;
}
