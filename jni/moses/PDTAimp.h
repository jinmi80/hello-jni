...
protected:
 ...
 public:
  ...
  GetTargetPhraseCollection(Phrase const &src) const {
	LOGE("[mgjang] in GetTargetPhraseCollection function\n");
    CHECK(m_dict);
    ...
    std::vector<std::string> wacands;
		LOGA("[mgjang] before call GetTargetCandidates\n");	
    m_dict->GetTargetCandidates(srcString,cands,wacands);
    if(cands.empty()) {
		LOGA("[mgjang] cands is empty\n");
      return 0;
    }

	LOGA("[mgjang] multiple models broken here\n"); 
  ...
  void Create(const std::vector<FactorType> &input
	...
	LOGE("[mgjang] in Create function [PDTAimp.h]\n");
	LOGE("[mgjang] weight.size = %d, m_numInputScores = %d\n",weight.size(), m_numInputScores);
    // set my members
    m_dict=new PhraseDictionaryTree(weight.size()-m_numInputScores);
    m_input=input;
    m_output=output;
    m_languageModels=&languageModels;
	LOGE("[mgjang] 1 in Create function [PDTAimp.h]\n");

    const StaticData &staticData = StaticData::Instance();
    m_dict->NeedAlignmentInfo(staticData.NeedAlignmentInfo());

	LOGE("[mgjang] 2 in Create function [PDTAimp.h]\n");

    std::string binFname=filePath+".binphr.idx";
	LOGE("[mgjang] binFname is %s [PDTAimp.h]\n", binFname.begin());
    if(!FileExists(binFname.c_str())) {
	LOGE("[mgjang] 3 in Create function [PDTAimp.h]\n");		
	...
	LOGE("[mgjang] 4 in Create function [PDTAimp.h]\n");	
    TRACE_ERR( "reading bin ttable\n");
//		m_dict->Read(filePath);
    bool res=m_dict->Read(filePath);
	LOGE("[mgjang] 5 in Create function [PDTAimp.h]\n");

    if (!res) {
	LOGE("[mgjang] 6 in Create function [PDTAimp.h]\n");		
      std::stringstream strme;
      strme << "bin ttable was read in a wrong way\n";
      UserMessage::Add(strme.str());
      exit(1);
    }
	LOGE("[mgjang] 7 in Create function [PDTAimp.h]\n");
  }
