...
bool StaticData::LoadDataStatic(Parameter *parameter, const std::string &execPath) {
	LOGE("[mgjang] in LoadDataStatic\n");
	LOGE("[mgjang] execPath : %s\n", execPath.begin());
...
bool StaticData::LoadData(Parameter *parameter)
{
  ...
  LOGE("[mgjang] after to cube or not to cube\n");
  ...
  LOGE("[mgjang] after load XXX Parameters\n");
  ...
  LOGE("[mgjang] before n-best factor\n");
  ...
  LOGE("[mgjang] before lattice-samples\n");
  ...
  if (m_parameter->GetParam("lattice-samples").size() ==2 ) {
	...
	LOGE("[mgjang] after lattice-samples\n");
  ...
  LOGE("[mgjang] before  search graph\n");
  ...
  if (m_parameter->GetParam("output-search-graph").size() > 0) {
    ...
    LOGE("[mgjang] after  search graph\n");
  ...
  LOGE("[mgjang] after extended format\n");
  ...
  LOGE("[mgjang] after HAVE_PROTOBUF\n");  
#endif
  ...
  LOGE("[mgjang] after output-unknowns\n");
  ...
  LOGE("[mgjang] after input factors\n");
  ...
  if (m_parameter->isParamSpecified("translation-details")) {
    ...
    LOGE("[mgjang] after additional output\n");
  ...
  if (m_parameter->GetParam("stack-diversity").size() > 0) {
    ...
    LOGE("[mgjang] after m_minHypoStackDiversity = 0\n");
  ...
  LOGE("[mgjang] before lattice mbr\n");
  ...
  LOGE("[mgjang] before consensus decoding\n");
  //consensus decoding
  ...
  LOGE("[mgjang] after  consensus decoding\n");
  ...
  LOGE("[mgjang] before load models\n");	
  if (!LoadLexicalReorderingModel()) return false;
  LOGE("[mgjang] after load LoadLexicalReorderingModel\n");	  
  if (!LoadLanguageModels()) return false;
    LOGE("[mgjang] after load LoadLanguageModels\n");	
  if (!LoadGenerationTables()) return false;
    LOGE("[mgjang] after load LoadGenerationTables\n");	
  if (!LoadPhraseTables()) return false;
    LOGE("[mgjang] after load LoadPhraseTables\n");	
  if (!LoadGlobalLexicalModel()) return false;
    LOGE("[mgjang] after load LoadGlobalLexicalModel\n");	
  if (!LoadGlobalLexicalModelUnlimited()) return false;
    LOGE("[mgjang] after load LoadGlobalLexicalModelUnlimited\n");	
  if (!LoadDecodeGraphs()) return false;
    LOGE("[mgjang] after load LoadDecodeGraphs\n");	
  if (!LoadReferences()) return  false;
    LOGE("[mgjang] after load LoadReferences\n");	
  if (!LoadDiscrimLMFeature()) return false;
    LOGE("[mgjang] after load LoadDiscrimLMFeature\n");	
  if (!LoadPhrasePairFeature()) return false;
    LOGE("[mgjang] after load LoadPhrasePairFeature\n");	
  if (!LoadPhraseBoundaryFeature()) return false;
    LOGE("[mgjang] after load LoadPhraseBoundaryFeature\n");	
  if (!LoadPhraseLengthFeature()) return false;
    LOGE("[mgjang] after load LoadPhraseLengthFeature\n");	
  if (!LoadTargetWordInsertionFeature()) return false;
    LOGE("[mgjang] after load LoadTargetWordInsertionFeature\n");	
  if (!LoadSourceWordDeletionFeature()) return false;
    LOGE("[mgjang] after load LoadSourceWordDeletionFeature\n");	
  if (!LoadWordTranslationFeature()) return false;
  LOGE("[mgjang] after load models\n");	
...
{
	LOGE("[mgjang] in LoadLanguageModels function\n");
    ...
    for(size_t i=0; i<lmVector.size(); i++) {
      ...
      if (languageModelsLoaded.find(lmVector[i]) != languageModelsLoaded.end()) {
        ...
        if (token.size() != 4 && token.size() != 5 ) {
		  ...
		  LOGE("[mgjang] Expected format 'LM-TYPE FACTOR-TYPE NGRAM-ORDER filePath [mapFilePath (only for IRSTLM)]\n");
          return false;
        }
		...
		LOGE("[mgjang] languageModelFile : %s\n", languageModelFile.begin());
        if (token.size() == 5) {
          if (lmImplementation==IRST)
          {
            languageModelFile += " " + token[4];
		  	LOGE("[mgjang] IRST token\n");
          }
          else {
            UserMessage::Add("Expected format 'LM-TYPE FACTOR-TYPE NGRAM-ORDER filePath [mapFilePath (only for IRSTLM)]'");
		  	LOGE("[mgjang]2 Expected format 'LM-TYPE FACTOR-TYPE NGRAM-ORDER filePath [mapFilePath (only for IRSTLM)]\n");
            return false;
          }
        }
        IFVERBOSE(1)
        ...
        if (lm == NULL) {
		  ...
		  LOGE("[mgjang] no LM created. We probably don't have it compiled\n");
          return false;
        }
