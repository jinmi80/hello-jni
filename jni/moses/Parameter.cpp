...
bool Parameter::LoadParam(const string &filePath)
{
  ...
  LOGE("[mgjang] in LoadParam(3arg)\n");
  LOGE("[mgjang] argv[0] %s\n", argv[0]);
  LOGE("[mgjang] argv[1] %s\n", argv[1]);
  LOGE("[mgjang] argv[2] %s\n", argv[2]);
...
bool Parameter::LoadParam(int argc, char* argv[])
{
  ...
  if ( (configPath = FindParam("-f", argc, argv)) == ""
    ...
    cerr << endl;
	LOGE("[mgjang] No configuration file was specified.  Use -config or -f\n");
    return false;
  } else {
    if (!ReadConfigFile(configPath)) {
      UserMessage::Add("Could not read "+configPath);
		//LOGE("[mgjang] Could not read %s\n", std::convertStringToChar(configPath));	  
		LOGE("[mgjang] Could not read %s\n", configPath.begin());	  
      return false;
    }
  }
...
//	LOGE("[mgjang] - paramShortName : %s, paramName : %s\n", paramShortName.begin(), paramName.begin());
  }

  LOGE("[mgjang] logging of parameters that were set in either config or switch\n");
  ...
  LOGE("[mgjang] check for illegal parameters\n");
...
bool Parameter::Validate()
{
  ...
  LOGE("[mgjang] in Validate function\n");
  ...
  for (iterParams = m_setting.begin(); iterParams != m_setting.end(); ++iterParams) {
    ...
    if (m_valid.find(key) == m_valid.end())
    {
      ...
      noErrorFlag = false;
	  LOGE("[mgjang]Unknown parameter\n");
    }
  }
  
  LOGE("[mgjang] in Validate function // required parameters\n");
  LOGE("[mgjang] m_setting. ttable-file size : %d\n", m_setting["ttable-file"].size());
  LOGE("[mgjang] m_setting. ttable-file string : %s\n", m_setting["ttable-file"][0].begin());
  // required parameters
  if (m_setting["ttable-file"].size() == 0) {
    UserMessage::Add("No phrase translation table (ttable-file)");
  LOGE("[mgjang] No phrase translation table (ttable-file)\n");	
    noErrorFlag = false;
  }

  LOGE("[mgjang] 1 in Validate function\n");
  if (m_setting["lmodel-dub"].size() > 0) {
    if (m_setting["lmodel-file"].size() != m_setting["lmodel-dub"].size()) {
  	...
  	LOGE("[mgjang]2 in Validate function\n");	  
      noErrorFlag = false;
    }
  }

  LOGE("[mgjang]3 in Validate function\n");
  ...
  if (m_setting["lmodel-file"].size() * (m_setting.find("lmodel-oov-feature") != m_setting.end() ? 2 : 1)
    ...
    UserMessage::Add(errorMsg.str());
  LOGE("[mgjang]4 in Validate function\n");	
    noErrorFlag = false;
  }

  // do files exist?
  LOGE("[mgjang]5 in Validate function\n");
  ...
  LOGE("[mgjang] end of validate function\n");  
  return noErrorFlag;
}
  ...
  for (iter = pathVec.begin() ; iter != pathVec.end() ; ++iter) {
    ...
    if (tokenizeIndex >= vec.size()) {
      ...
      errorMsg << "Expected at least " << (tokenizeIndex+1) << " tokens per entry in '"
               ...
               << vec.size();
      UserMessage::Add(errorMsg.str());
	    LOGE("[mgjang]1 FileExist return false\n");	  
      return false;
    }
    ...
    if(!fileFound) {
      ...
      UserMessage::Add(errorMsg.str());
	  LOGE("[mgjang]2 FileExist return false\n");	
	  return false;
    }
  }

  LOGE("[mgjang]3 FileExist return true\n");	  
  return true;
}
...
{
	LOGE("[mgjang] in ReadConfigFile\n");
	LOGE("[mgjang] filepath is %s\n", filePath.begin());
//	filePath.add("\moses.kj.ini");
  ...
  LOGE("[mgjang] end of ReadConfigFile\n");
  return true;
}
