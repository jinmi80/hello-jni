...
#endif
#ifndef LOGA
#define LOGA(...) __android_log_print(ANDROID_LOG_ERROR  , "libnav", __VA_ARGS__)
#endif
...
void TranslationOptionCollection::CreateTranslationOptions()
{
  ...
  for (size_t graphInd = 0 ; graphInd < decodeGraphList.size() ; graphInd++) {
    if (decodeGraphList.size() > 1) {
	  ...
	  LOGA("[mgjang] Creating translation options from decoding graph. graphInd is %d \n", graphInd);
    }
    ...
    for (size_t startPos = 0 ; startPos < size; startPos++) {
      ...
      size_t maxSizePhrase = StaticData::Instance().GetMaxPhraseLength();
	  LOGA("[mgjang] startPos : %d, maxSize : %d, maxSizePhrase : %d \n", startPos, maxSize, maxSizePhrase);
      maxSize = std::min(maxSize, maxSizePhrase);
	  LOGA("[mgjang] startPos : %d, maxSize : %d, maxSizePhrase : %d \n", startPos, maxSize, maxSizePhrase);

      // ... and that end at endPos
      for (size_t endPos = startPos ; endPos < startPos + maxSize ; endPos++) {
	  LOGA("[mgjang] endPos : %d\n", endPos);	  	
        if (graphInd > 0 && // only skip subsequent graphs
		  ...
		  LOGA("[mgjang] No backoff to graph. do continue\n");	 
          // do not create more options
          continue;
        }

        // create translation options for that range
  	LOGE("[MGJANG] before CreateTranslationOptionsForRange function. \n");        
        CreateTranslationOptionsForRange( decodeGraph, startPos, endPos, true, graphInd);
		  LOGE("[MGJANG] after CreateTranslationOptionsForRange function. \n");
      }
    }
  }
  ...
  LOGE("[MGJANG] after IncorporateDLMScores function. \n");

  ProcessUnknownWord();
  LOGE("[MGJANG] after ProcessUnknownWord function. \n");

  // Prune
  Prune();
  LOGE("[MGJANG] after Prune function. \n");

  Sort();
  LOGE("[MGJANG] after Sort function. \n");

  // future score matrix
  CalcFutureScore();
  LOGE("[MGJANG] after CalcFutureScore function. \n");

  // Cached lex reodering costs
  CacheLexReordering();
  LOGE("[MGJANG] after CacheLexReordering function. \n");

  // stateless feature scores
  PreCalculateScores();

  LOGE("[MGJANG] end of CreateTranslationOptions function. \n");  
}
...
void TranslationOptionCollection::CreateTranslationOptionsForRange(
  ...
  , size_t graphInd)
{
	LOGA("[mgjang] in CreateTranslationOptionsForRange\n");
  if ((StaticData::Instance().GetXmlInputType() != XmlExclusive) || !HasXmlOptionsOverlappingRange(startPos,endPos)) {
  		LOGA("[mgjang] 1 in CreateTranslationOptionsForRange\n");
    ...
    if (useCache) {
		LOGA("[mgjang] useCache is true\n");
      const WordsRange wordsRange(startPos, endPos);
      sourcePhrase = new Phrase(m_source.GetSubString(wordsRange));

	  LOGA("[mgjang] before FindTransOptListInCache\n");
    ...
    if (!skipTransOptCreation) {
	  ...
	  LOGA("[mgjang] after ProcessInitialTranslation\n");

      // do rest of decode steps
      int indexStep = 0;
	  LOGA("[mgjang] 1 indexStep is %d\n", indexStep);
      for (++iterStep ; iterStep != decodeGraph.end() ; ++iterStep) {
        ...
        for (iterPartialTranslOpt = partTransOptList.begin() ; iterPartialTranslOpt != partTransOptList.end() ; ++iterPartialTranslOpt) {
			LOGA("[mgjang] in iterPartialTransOpt loop\n");			
		...
		LOGA("[mgjang] totalEarlyPruned is %d\n", totalEarlyPruned);
        delete oldPtoc;
        oldPtoc = newPtoc;
        indexStep++;
		LOGA("[mgjang] 2 indexStep is %d\n", indexStep);
      } // for (++iterStep
  ...
  if (graphInd == 0 && StaticData::Instance().GetXmlInputType() != XmlPassThrough && HasXmlOptionsOverlappingRange(startPos,endPos)) {
  	LOGE("[mgjang] before CreateXmlOptionsForRange\n");
