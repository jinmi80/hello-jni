...
{
	LOGE("[mgjang] in SearchNormal function %s, %d\n", __FILE__, __LINE__);
...
void SearchNormal::ProcessSentence()
{
  ...
  for (iterStack = m_hypoStackColl.begin() ; iterStack != m_hypoStackColl.end() ; ++iterStack) {
    ...
    if (_elapsed_time > staticData.GetTimeoutThreshold()) {
      ...
      interrupted_flag = 1;
	  LOGE("[mgjang] Decoding is out of time %s, %d\n", __FILE__, __LINE__);
      return;
    }
  ...
  LOGE("[mgjang] End of foo loop %s, %d\n", __FILE__, __LINE__);  
  ...
  LOGE("[mgjang] End of ProcessSentence function %s, %d\n", __FILE__, __LINE__);  
}
