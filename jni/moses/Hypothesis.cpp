...
#endif
#ifndef LOGC
#define LOGC(...) __android_log_print(ANDROID_LOG_ERROR  , "libnav", __VA_ARGS__)
#endif
...
{
  	LOGE("[mgjang] in create function\n");
#ifdef USE_HYPO_POOL
  LOGE("[mgjang] USE_HYPO_POOL\n");
  Hypothesis *ptr = s_objectPool.getPtr();
  return new(ptr) Hypothesis(manager, m_source, emptyTarget);
#else
  LOGE("[mgjang] not USE_HYPO_POOL\n");
